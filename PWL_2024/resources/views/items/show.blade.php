...
<body>
    ...
    <!-- // Menampilkan pesan sukses jika ada -->
        <p>{{ session('success') }}</p> 
        <!-- // Menampilkan pesan sukses yang tersimpan dalam session -->
    @endif
    <a href="{{ route('items.create') }}">Add Item</a> 
    <!-- // Link untuk menambahkan item baru -->
    <ul> 
        <!-- // Membuat daftar item dalam format unordered list -->
        ...
        <!-- // Melakukan iterasi melalui daftar item -->
            <li>
                ...
                <form action="{{ route('items.destroy', $item) }}" method="POST" style="display:inline;"> 
                    ...
                    @method('DELETE') 
                    <!-- // Menggunakan metode HTTP DELETE untuk menghapus item -->
